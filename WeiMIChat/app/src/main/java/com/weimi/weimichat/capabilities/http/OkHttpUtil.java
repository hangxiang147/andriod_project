...
public class    OkHttpUtil {
    ...
    class TRequestCallBack<T> implements Callback {
        ...
        public void onResponse(Response response) throws IOException {
            ...
            if (response.isSuccessful()) {
                ...
                final T object = GsonHelper.toTypeForResult(result, clazz);
                int code = -1;
                if (res != null && res instanceof BaseResp) {
                    code = ((BaseResp) res).getCode();
                    switch (code) {
                        case 200:
                            if(null == object){
                                postSucessMsg(res);
                            ...
                            break;
                        case 400:
                            notifyMsg = ((BaseResp) res).getMassege();
                            switch (notifyMsg){
                                case "4":
                                    notifyMsg = PWD_ERROR;
                                    postErrorMsg();
                                    break;
                                case "5":
                                    notifyMsg = USERANEM_ERROR;
                                    postErrorMsg();
                                    break;
                                case "8":
                                    notifyMsg = REGISTER_ERROR;
                                    postErrorMsg();
                                    break;
                                case "9":
                                    notifyMsg = EMAIL_EXIST;
                                    postErrorMsg();
                                    break;
                                case "10":
                                    notifyMsg = INVIATE_CODE_ERROR;
                                    postErrorMsg();
                                    break;
                                case "11":
                                    notifyMsg = EMAIL_ERROR;
                                    postErrorMsg();
                                    break;
                                case "12":
                                    notifyMsg = ACTIVE_EMAIL;
                                    postErrorMsg();
                                    break;
                                case "13":
                                    notifyMsg = EMAIL_ERROR;
                                    ...
                                    break;
                                case "15":
                                    notifyMsg = EMAIL_CODE_ERROR;
                                    postErrorMsg();
                                    break;
                                case "16":
                                    notifyMsg = RONG_ERROR;
                                    postErrorMsg();
                                    break;
                                default:
                                    notifyMsg = SERVER_ERROR;
   ...
   // public static String SERVER_ERROR = "您的网络状况不佳，请检查网络连接";

    public static String USERANEM_ERROR = "账号不存在";

    public static String PWD_ERROR = "密码错误";

    public static String REGISTER_ERROR = "用户名已存在";

    public static String EMAIL_EXIST = "邮箱已存在";

    public static String EMAIL_ERROR = "邮箱不可用";

    public static String INVIATE_CODE_ERROR = "邀请码不存在";

    public static String ACTIVE_EMAIL = "请先激活邮箱，再登录";

    public static String EMAIL_CODE_ERROR = "验证码输入有误";

    public static String RONG_ERROR = "融云接口异常";
